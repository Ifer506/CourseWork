...
public class Week6
{
    ...
    static int no1,no2,no3,no4,count=0;
    ...
    public static void calculate()
    {
        ...
        for(int i=0;i<permuts.size();i++)
        {
            ...
            if(no3==no1+no2 && getLengthOfInt(no1)==s1.length() && getLengthOfInt(no2)==s2.length() && getLengthOfInt(no3)==s3.length() && getLengthOfInt(no4)==s4.length() && count<1)
            {
                solutionfound=true;
                System.out.println(s1+":"+no1+"  "+s2+":"+no2+" "+s3+":"+no3+"   "+s4+":"+no4  );
                count++;
            }
        }
